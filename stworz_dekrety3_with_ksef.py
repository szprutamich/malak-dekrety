...
def convert_file():
    ...
    ksef_row_formatting = get_formatting(workbook, is_ksef=True)
    ...
    for idx, decree in enumerate(filtered):
        if idx != 0 and idx % 3 == 0:
            ksef_alignment = 2 if decree['nr_ksef'] is not None else 0
            start = start + rows_per_page // 4 * (
                        (max_rows_from_three_operations + 3 + ksef_alignment) // (rows_per_page // 4 - 1) + 1)
        ...
        if start - page_breaks[-1] > rows_per_page:  # adjust start to page start
            ...
            page_breaks.append(start - 1)
        write_decree(worksheet, decree, start, columns[idx % 3], formatting, ksef_row_formatting, money_formatting)
...
def write_decree(worksheet, decree, start, columns, formatting, ksef_row_formatting, money_formatting):
    is_ksef = decree['nr_ksef'] is not None
    ksef = 2 if is_ksef else 0
    ...
    if is_ksef:
        worksheet.merge_range(columns[0] + str(start + 2) + ':' + columns[0] + str(start + 3), 'Nr KSeF',
                              ksef_row_formatting)
        worksheet.merge_range(columns[1] + str(start + 2) + ':' + columns[2] + str(start + 3), decree['nr_ksef'],
                              ksef_row_formatting)
    worksheet.write(columns[0] + str(start + 2 + ksef), 'Konto', formatting)
    worksheet.write(columns[1] + str(start + 2 + ksef), 'Wn', formatting)
    worksheet.write(columns[2] + str(start + 2 + ksef), 'Ma', formatting)
    for idx, row in enumerate(decree['rows']):
        worksheet.write(columns[0] + str(start + 3 + idx + ksef), row['account'], formatting)
        worksheet.write(columns[1] + str(start + 3 + idx + ksef), get_currency_value(row['wn']), money_formatting)
        worksheet.write(columns[2] + str(start + 3 + idx + ksef), get_currency_value(row['ma']), money_formatting)


def get_formatting(workbook, is_ksef=False):
...
def parse_table(table, month, year):
    decree = {'symbol': None, 'date': None, 'input_date': None, 'nr_ksef': None, 'number': None, 'rows': []}
    if int(table.attrib['cols']) == 5:
        parse_summary_table(table)
    if int(table.attrib['cols']) != 12:
        return None
    for idx, row in enumerate(table):
        if row.tag == 'row':
            ...
            if decree['nr_ksef'] is None:
                decree['nr_ksef'] = row[nr_ksef_idx][0].text
            if decree['number'] is None:
                decree['number'] = row[nr_idx][0].text
            decree_row = {'account': row[account_idx][0].text, 'wn': row[wn_idx][0].text, 'ma': row[ma_idx][0].text}
            decree['rows'].append(decree_row)
            if decree['symbol'] is None:
                if row[symbol_idx][0].text is not None:
                    decree['symbol'] = row[symbol_idx][0].text.upper()
                else:
                    decree['symbol'] = ""
    return decree
